...
description: Registers existing React components with Genui so AI can render them. Use when user wants to make their existing components available to AI, register components for generative UI, convert React components to Genui components, or mentions /add-components-to-registry. For creating NEW components, see the components skill. For project setup, use add-to-existing-project or generative-ui skills.
---

# Add Components to Registry

Convert existing React components into Genui-registered components that AI can render.
...
4. **Add to registry** - Update lib/genui.ts
...
```tsx
// lib/genui.ts
import { GenuiComponent } from "@workspace/react";
