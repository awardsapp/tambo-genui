...
description: Handles Genui component streaming, loading states, and persistent state. Use when implementing streaming UI feedback, tracking prop streaming status, managing partial props, or persisting component state across sessions with useGenuiStreamStatus or useGenuiComponentState.
---
...
```tsx
import { useGenuiComponentState, useGenuiStreamStatus } from "@workspace/react";

function EditableCard({ title: streamedTitle }: { title?: string }) {
  const [title, setTitle, { isPending, flush }] = useGenuiComponentState(
    "title",
    "",
  );
...
### useGenuiComponentState API

```tsx
const [value, setValue, meta] = useGenuiComponentState(
