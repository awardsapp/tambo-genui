...
// Use npx to ensure we get the latest version of genui
const args = ["-y", "genui@latest", "create-app", ...process.argv.slice(2)];
...
child.on("error", (err: NodeJS.ErrnoException) => {
  if (err?.code === "ENOENT") {
    console.error(
      "create-genui-app: `npx` was not found on your PATH. Install Node.js (which provides npx) or ensure `npx` is available, then retry.",
    );
    process.exit(127);
    return;
  }
  ...
  console.error(
    `create-genui-app: failed to launch 'npx': ${err?.message ?? "unknown error"}`,
  );
  process.exit(1);
});
