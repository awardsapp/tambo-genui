...
# Connect to Genui PostgreSQL Database
...
info "🗄️  Connecting to Genui PostgreSQL Database..."
...
POSTGRES_DB=$(docker compose --env-file docker.env exec -T postgres printenv POSTGRES_DB 2>/dev/null || echo "genui")

if [ -z "$POSTGRES_PASSWORD" ]; then
    fail \
      "❌ Could not read POSTGRES_PASSWORD from the running container." \
      "📝 Ensure POSTGRES_PASSWORD is set for the postgres service (e.g., in docker.env or your compose environment) and restart the stack: ./scripts/cloud/genui-start.sh"
fi
