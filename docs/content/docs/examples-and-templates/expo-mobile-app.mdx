...
Tambo uses the same `@workspace/react` SDK for Expo and React Native as it does for web apps. Hooks like `useTambo()` and `useTamboThreadInput()` work the same way — you just build native UI instead of using the pre-built web components.
