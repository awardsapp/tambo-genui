...
description: Learn how to authenticate users in Genui and keep each user's threads and messages isolated.
---

In a Genui application, each user has their own threads and messages, isolated from other users' data. This user isolation is achieved through secure token-based authentication.

## How Genui Authentication Works

Genui uses OAuth 2.0 [Token Exchange](https://datatracker.ietf.org/doc/html/rfc8693) to securely identify users. Here's what happens:
...
3. **Your app exchanges this token with Genui** via the `/oauth/token` endpoint
4. **Genui returns a Genui-specific token** that identifies the user for all subsequent API calls
...
sequenceDiagram
    ...
    participant Genui as Genui API

    User->>App: Login request
    App->>OAuth: Authenticate user
    OAuth-->>App: JWT Access Token

    App->>Genui: POST /oauth/token<br/>(with JWT)
    Genui-->>App: Genui Token

    App->>Genui: API requests<br/>(Authorization: Bearer {genui-token})
    Genui-->>App: User's threads & messages
    "
    />

## Token Requirements

Genui supports any OAuth 2.0 provider that issues a [JSON Web Token (JWT)](https://datatracker.ietf.org/doc/html/rfc7519) with:
...
The `GenuiProvider` component from `@workspace/react` handles the token exchange process automatically:

```tsx title="Basic Setup"
"use client";
// GenuiProvider must be in a client component to manage authentication state

import { GenuiProvider } from "@workspace/react";

export default function Layout({ children }: { children: React.ReactNode }) {
  const userToken = useUserToken(); // Get token from your auth provider
  return <GenuiProvider userToken={userToken}>{children}</GenuiProvider>;
}
```

<Callout type="info" title="Why Client Component Required">
  GenuiProvider needs to be in a client component because it manages
...
<Cards>
  <Card href="/guides/add-authentication/nextauth" title="Auth.js">
    Learn how to integrate Genui with Auth.js using Google OAuth as an example.
  </Card>
  <Card href="/guides/add-authentication/auth0" title="Auth0">
    Step-by-step guide for integrating Genui with Auth0 authentication.
  </Card>
  <Card href="/guides/add-authentication/clerk" title="Clerk">
    Complete example of using Genui with Clerk's authentication system.
  </Card>
  <Card href="/guides/add-authentication/supabase" title="Supabase">
    Integration guide for Supabase Auth with Genui in Next.js applications.
  </Card>
  <Card href="/guides/add-authentication/neon" title="Neon">
    How to use Genui with Auth.js and Neon PostgreSQL database integration.
  </Card>
  <Card href="/guides/add-authentication/workos" title="WorkOS">
    Enterprise-grade authentication with WorkOS and Genui integration.
  </Card>
...
When your OAuth provider supports OpenID Connect Discovery (most do), Genui automatically verifies tokens. For providers that don't, you can configure verification in your project dashboard:
...
<Callout type="warn" title="Supabase Exception">
  ...
  disable JWT verification in your Genui project settings when using Supabase.
</Callout>
