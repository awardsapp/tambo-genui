...
MCP tools automatically support rich content responses. When tools return content arrays (text, images, and other media), Genui passes them directly to the AI without converting to plain text. This enables tools to seamlessly return images, formatted content, and other rich media.
...
Prompt parameters are not yet supported. While the MCP specification allows prompts to accept customization parameters, Genui's current implementation inserts all prompts as-is using their default values. Parameter support is planned for a future release.
...
- **Server-side resources:** `@serverKey:serverSidePrefix:resourceUri`
  - Example: `@linear:genui:issue://TAM-123`
...
- The resource reference is sent to Genui's API
...
- The complete content is sent to Genui's API
...
Genui automatically displays sampling interactions in the UI as expandable dropdowns within tool information panels, showing the messages exchanged between the MCP server and the LLM.
...
sequenceDiagram
    ...
    User->>Genui: "What's the weather in SF?"
    Genui->>MCPServer: Call weather tool
    MCPServer->>MCPServer: Fetch raw weather data
    MCPServer->>Genui: Sampling request: "Summarize this weather data"
    Genui->>LLM: Generate summary
    LLM->>Genui: Summary response
    Genui->>MCPServer: Sampling result
    MCPServer->>Genui: Tool result with summary
    Genui->>User: Display formatted weather
```
...
Sampling currently only works with server-side MCP connections. Client-side MCP servers cannot make sampling requests at this time. This limitation exists because sampling requests need to be processed through Genui's backend infrastructure for proper security, rate limiting, and LLM access control.
