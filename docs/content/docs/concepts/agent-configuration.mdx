...
description: Configure your Genui agent's behavior, LLM provider, MCP servers, and authentication settings
---

Messages sent from your app to Genui are processed by the Genui agent, which you can configure through your project settings page. You can add custom instructions to change Genui's behavior and personality, configure the underlying LLM model and its options, connect MCP servers to extend capabilities, and set up user authentication.

## Custom Instructions

Custom instructions define how Genui behaves and responds to users.

When you add custom instructions, you're customizing Genui's "system prompt". This is where you define the agent's role (is it a support agent? a creative partner? a data analyst?), set the tone and personality (formal or casual? friendly or professional?), establish behavioral guidelines for handling different types of requests, and give your AI a name or identity to help users understand who they're talking to.
...
The LLM configuration determines which AI model powers your Genui agent and how it behaves. You can change the provider, select specific models, and fine-tune parameters to match your application's needs.

### Choosing a Provider and Model

Genui integrates with major AI providers including OpenAI, Anthropic, Google, Groq, and Mistral. Each provider offers different models optimized for various use cases. See the [Models and Providers](/reference/llm-providers) page for a complete list of available providers and their capabilities. You can also use any OpenAI-compatible model by selecting the "OpenAI Compatible" provider and specifying a custom endpoint.
...
Skills are reusable, toggleable instruction sets that extend your agent's capabilities. Unlike custom instructions which define general behavior, skills are modular and run inside the LLM provider's sandboxed execution environment with code execution support.

You can create skills through the dashboard or import them from markdown files. Each skill has a name, description, and instructions. Skills can be individually enabled or disabled without affecting other skills or custom instructions.

Skills are supported by select OpenAI (GPT-5.2 and newer) and Anthropic (Claude Haiku 4.5, Sonnet 4.5+, Opus 4.5+) models. If your project uses an unsupported model, skills are stored but not active at inference time. See the [full model support list](/concepts/skills#model-support).
...
Model Context Protocol servers extend your Genui agent's capabilities by providing access to external tools, data sources, and services. You can connect MCP servers directly from your project settings page, making their capabilities available to all users of your application.
...
User authentication determines how Genui identifies and isolates users in your application. Each user has their own threads and messages, kept separate from other users' data through secure token-based authentication.
