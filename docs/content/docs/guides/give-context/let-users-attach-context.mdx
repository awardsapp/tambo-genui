...
- A Genui application with `GenuiProvider` configured
- Understanding of React hooks

## Step 1: Import the Hook

Import the `useGenuiContextAttachment` hook:
...
The `useGenuiContextAttachment` hook returns:
...
function StagedContextDisplay() {
  const { attachments, removeContextAttachment } = useGenuiContextAttachment();
