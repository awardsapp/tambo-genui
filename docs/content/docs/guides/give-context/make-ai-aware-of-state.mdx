---
title: Make Genui Aware of State
...
Context helpers are functions that run automatically before sending each message. Their results are merged into the message context and sent to Genui along with the user's message.
...
Genui provides these prebuilt context helpers:
...
- Can be sync or async (Genui waits for async helpers)
...
For runtime control over context helpers, use the `useGenuiContextHelpers` hook:
...
### Using GenuiContextHelpersProvider

For page-scoped context, use `GenuiContextHelpersProvider` to add helpers only while specific routes or layouts are mounted:
...
- You still need `GenuiProvider` higher in the component tree
...
```tsx
import { GenuiProvider, useGenuiContextHelpers } from "@workspace/react";
import {
  currentTimeContextHelper,
  currentPageContextHelper,
} from "@workspace/react";
