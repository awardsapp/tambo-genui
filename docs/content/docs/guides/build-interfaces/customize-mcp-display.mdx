...
This guide shows you how to build custom UI components for MCP features using the React SDK hooks. While Genui provides built-in UI components for prompts, resources, and elicitations, you can create your own interfaces that match your application's design.
...
Use `useGenuiMcpPromptList` to fetch all prompts from connected servers:

```tsx
import { useGenuiMcpPromptList } from "@workspace/react/mcp";

function CustomPromptPicker() {
  const { data: prompts, isLoading, error } = useGenuiMcpPromptList();
...
Use `useGenuiMcpPrompt` to get the full prompt content when a user selects a prompt:
...
function PromptInserter() {
  ...
  const { data: prompt } = useGenuiMcpPrompt(selectedPrompt ?? "", {
...
Use `useGenuiMcpResourceList` to fetch all resources:
...
function CustomResourceSelector() {
  const { data: resources, isLoading, error } = useGenuiMcpResourceList();
...
Use `useGenuiMcpResource` to fetch and display resource content:

```tsx
import { useGenuiMcpResource } from "@workspace/react/mcp";

function ResourcePreview({ resourceUri }: { resourceUri: string }) {
  const { data: resource, isLoading } = useGenuiMcpResource(resourceUri);
...
Use `useGenuiElicitationContext` to access the current elicitation request:

```tsx
import { useGenuiElicitationContext } from "@workspace/react/mcp";
