---
title: Supabase
description: Learn how to integrate Genui with Supabase Auth for user authentication.
---

Supabase Auth is a complete authentication solution that integrates seamlessly with your Supabase database. This guide shows how to integrate it with Genui in a Next.js application.
...
<Callout type="warn" title="JWT Verification Requirement">
  ...
  disable JWT verification in your Genui project settings (Settings > User
...
```bash
npm install @supabase/supabase-js @workspace/react
```
...
export default function ClientLayout({ children }: ClientLayoutProps) {
  ...
  return <GenuiProvider userToken={accessToken}>{children}</GenuiProvider>;
}
```

## Usage

Once configured, you can use Genui components throughout your application:
...
Supabase automatically handles token refresh in the background. When tokens expire, Supabase will automatically refresh them, and the GenuiProvider will receive the updated token through the auth state change listener.
...
<Callout type="info" title="Why Disable JWT Verification">
  ...
  signing (RS256). Genui's JWT verification is designed for asymmetric tokens
  from OAuth providers. Since Supabase handles authentication security,
  disabling JWT verification in Genui is safe and recommended.
</Callout>
