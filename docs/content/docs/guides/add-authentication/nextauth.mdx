---
title: Auth.js
description: Learn how to integrate Genui with Auth.js for user authentication.
---

Auth.js (formerly NextAuth.js) is a complete authentication solution for Next.js applications. This guide demonstrates integration with Genui using Google as the OAuth provider.
...
```bash
npm install next-auth @auth/core @workspace/react
```
...
<Callout type="info" title="GitHub and Other Non-OIDC Providers">
  ...
  provider, select the **GitHub** preset in the Genui dashboard OAuth settings.
