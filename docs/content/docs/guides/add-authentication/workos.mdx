---
title: WorkOS
description: Learn how to integrate Genui with WorkOS for user authentication.
---

WorkOS is an enterprise-grade authentication and user management platform that provides features like SSO, SCIM provisioning, and directory sync. This guide shows how to integrate it with Genui in a Next.js application.
...
```bash
npm install @workos-inc/node @workspace/react
```
...
export default function ClientLayout({ children }: ClientLayoutProps) {
  ...
  return (
    <GenuiProvider userToken={accessToken || undefined}>
      {children}
    </GenuiProvider>
  );
}
...
WorkOS provides several enterprise-grade features that work seamlessly with Genui:
...
<Callout type="info" title="Enterprise Token Security">
  ...
  user identification and organizational context that Genui needs.
</Callout>
