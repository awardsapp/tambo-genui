---
title: Neon
description: Learn how to integrate Genui with Auth.js using Neon as the database backend for session storage.
---

Auth.js (formerly NextAuth.js) is a complete authentication solution that can use various database adapters. This guide shows how to integrate Genui with Auth.js when using Neon as the database backend for session and user data storage.
...
```bash
npm install next-auth @auth/neon-adapter @neondatabase/serverless @workspace/react
```
...
export default function ClientLayout({ children }: ClientLayoutProps) {
  ...
  return (
    <GenuiProvider userToken={session?.accessToken}>{children}</GenuiProvider>
  );
}
```
