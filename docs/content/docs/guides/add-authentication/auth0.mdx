---
title: Auth0
description: Learn how to integrate Genui with Auth0 for user authentication.
---

Auth0 is a comprehensive identity and access management platform that provides secure authentication and authorization services. This guide shows how to integrate it with Genui in a Next.js application.
...
```bash
npm install @auth0/nextjs-auth0 @workspace/react
```
...
export default function ClientLayout({
  ...
  return (
    <UserProvider>
      <GenuiProvider userToken={userToken}>{children}</GenuiProvider>
    </UserProvider>
  );
}
```
...
function GenuiWrapper({ children }: { children: ReactNode }) {
...
export default function ClientLayout({ children }: ClientLayoutProps) {
  return (
    <UserProvider>
      <GenuiWrapper>{children}</GenuiWrapper>
    </UserProvider>
  );
}
```
...
<Callout type="info" title="Token Security">
  ...
  which is exactly what Genui needs for user identification. The tokens are
