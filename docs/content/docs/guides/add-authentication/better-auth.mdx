---
title: Better Auth
description: Learn how to integrate Genui with Better Auth for user authentication.
---

Better Auth is a modern authentication library that provides built-in support for multiple providers and plugins. This guide shows how to integrate it with Genui in a Next.js application.
...
```bash
npm install better-auth @workspace/react
```
...
export default function ClientLayout({ children }: ClientLayoutProps) {
  ...
  return <GenuiProvider userToken={session?.token}>{children}</GenuiProvider>;
}
```

## Usage

Once configured, you can use Genui components throughout your application. The authentication context is automatically handled:
...
Your Genui integration is now complete. The `GenuiProvider` will automatically:

- Exchange your Better Auth token for a Genui token
- Refresh the Genui token when it expires
- Handle authentication state changes
- Provide user isolation for all Genui API calls
