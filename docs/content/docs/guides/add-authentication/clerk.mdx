---
title: Clerk
description: Learn how to integrate Genui with Clerk for user authentication.
---

Clerk is a complete authentication and user management solution that handles authentication through middleware without requiring specific API routes. This guide shows how to integrate it with Genui.
...
```bash
npm install @clerk/nextjs @workspace/react
```
...
<Callout type="info" title="Token Format">
  ...
  is exactly what Genui needs for user identification. The token is
