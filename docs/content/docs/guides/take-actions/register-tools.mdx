---
title: Give Genui Access to Your Functions
description: This guide helps you register custom JavaScript functions as tools that Genui can call to retrieve data or take actions.
---

import LearnMore from "@/components/learn-more";
import { Wrench } from "lucide-react";

This guide shows you how to register custom JavaScript functions as tools so Genui can call them to retrieve data or take actions in response to user messages.

<LearnMore
  title="Learn more about tools"
  description="Understand what tools are and how they enable Genui to take actions"
  href="/concepts/tools"
  icon={Wrench}
/>
...
- GenuiProvider set up in your application
...
Wrap your function in a GenuiTool object with a schema. The `description` field helps Genui decide when to use your tool, and `.describe()` on schema fields helps Genui understand how to call it with the right parameters.

```tsx
import { GenuiTool } from "@workspace/react";
import { z } from "zod";
...
Write clear, specific descriptions so Genui knows when to call your tool and what values to pass.

## Step 3: Register with GenuiProvider
...
Genui can now call your tool when relevant to user messages.
...
export const productTool: GenuiTool = {
  name: "get_product",
...
Avoid `annotations.genuiStreamableHint: true` when your tool:
...
export const chartTool: GenuiTool = {
  name: "update_chart",
  ...
  outputSchema: z.void(),
  annotations: {
    genuiStreamableHint: true, // Enable streaming execution
  },
};
```
...
export const dashboardTool: GenuiTool = {
...
export const createUserTool: GenuiTool = {
  name: "create_user",
  ...
  annotations: { genuiStreamableHint: true }, // Don't do this!
};

// ✅ Good: State update tool - safe to call multiple times
export const updateFormTool: GenuiTool = {
  name: "update_form",
  ...
  outputSchema: z.void(),
  annotations: { genuiStreamableHint: true }, // Safe for repeated calls
};
```
