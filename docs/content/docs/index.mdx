...
description: Register components the Genui agent renders based on user messages.
icon: GraduationCap
---
...
Genui is an open-source toolkit for building AI-powered React apps. Register your components, and the agent decides which one to render based on user messages.
...
Self-host the backend or use Genui Cloud for fast deployment.
...
<GenuiProvider components={components} mcpServers={mcpServers}>
  <App />
</GenuiProvider>;
```
