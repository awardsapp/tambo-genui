...
The `@workspace/react` package exports TypeScript types and interfaces to help you build type-safe AI applications.

## GenuiTool

The `GenuiTool` interface defines the structure for registering tools with Genui.
...
The unique identifier for the tool. This is how Genui references the tool internally.
...
The `GenuiComponent` interface defines the structure for registering React components with Genui.
...
The state interface returned by the `useGenuiContextAttachment` hook.
...
Global stream status flags for a component during streaming. Returned by `useGenuiStreamStatus`.
...
Streaming status flags for individual component props. Returned by `useGenuiStreamStatus`.
...
Represents a component instance that can be interacted with by Genui. Extends `GenuiComponent`.
...
type ToolAnnotations = MCPToolAnnotations & {
  genuiStreamableHint?: boolean; // Safe to call repeatedly during streaming
};
```

The `genuiStreamableHint` property indicates that the tool is safe to be called repeatedly while a response is being streamed. This is typically used for read-only tools that do not cause side effects.
