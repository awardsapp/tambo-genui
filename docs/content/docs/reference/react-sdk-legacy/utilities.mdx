...
The `@workspace/react` package exports utility functions for common tasks like defining tools with full type inference and making components interactable.

## defineTool

Type-safe helper for defining Genui tools. Provides full type inference from your schema definitions.
...
| `annotations`        | `ToolAnnotations` | No       | Behavior hints (e.g., `genuiStreamableHint`)   |
| `transformToContent` | `function`        | No       | Transform result to content parts              |

### Schema Support

Genui uses the [Standard Schema](https://standard-schema.dev) specification, so you can use any compliant validator:
...
For tools that are safe to call repeatedly during streaming (typically read-only tools), use the `genuiStreamableHint` annotation:
...
const searchTool = defineTool({
  ...
  annotations: {
    genuiStreamableHint: true, // Safe for streaming
  },
...
Higher-Order Component that makes any component interactable by Genui. Interactable components can have their props and state modified by the AI during a conversation.
...
```tsx
import { withInteractable, useGenuiComponentState } from "@workspace/react";
import { z } from "zod";
...
const Task = ({
  ...
  const [isComplete, setIsComplete] = useGenuiComponentState(
    "isComplete",
    false,
  );
...
Genui provides pre-built context helpers that automatically provide useful information to the AI.
...
```tsx
import { currentPageContextHelper } from "@workspace/react";
...
```tsx
import { currentTimeContextHelper } from "@workspace/react";
...
Context helpers are configured on the `GenuiProvider`:
...
You can add/remove context helpers dynamically using `useGenuiContextHelpers`:
