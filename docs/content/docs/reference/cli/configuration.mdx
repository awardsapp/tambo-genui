...
description: See how the Genui CLI configures CSS and Tailwind for installed components, and how to customize or recreate the setup in your own project.
---

The Genui CLI automatically configures your CSS and Tailwind setup based on your project's Tailwind CSS version. This page explains what changes are made and how to configure them manually if needed.

## What Gets Configured

When you run Genui CLI commands (`full-send`, `add`, `update`, `upgrade`), the CLI:

1. **Detects your framework** (Next.js, Expo, Vite, or other) and **Tailwind CSS version** (v3 or v4)
2. **Sets up the Tailwind v4 build toolchain** if needed (see below)
3. **Updates your globals CSS** with required CSS variables
4. **Updates your `tailwind.config.ts`** (v3 only) with basic configuration
5. **Preserves your existing styles** and configuration

<Callout type="info" title="Automatic Detection">
  The CLI automatically detects your framework and Tailwind version from your
  project files and `package.json`. You don't need to specify which framework or
  Tailwind version you're using.
</Callout>

## Tailwind v4 Build Toolchain

Tailwind CSS v4 requires a build plugin to process CSS. The CLI sets this up automatically based on your framework:

### Vite

The CLI installs `@tailwindcss/vite` as a dev dependency and adds it to your Vite config:
...
For Next.js and other frameworks, the CLI installs `@tailwindcss/postcss` and creates a PostCSS config:
...
<Callout type="info" title="Idempotent Setup">
  The toolchain setup is idempotent. If the plugin is already configured, the
  CLI skips the setup. Running the CLI multiple times is safe.
</Callout>

## CSS Variables Added

Genui components require specific CSS variables to function properly. The CLI adds these in the appropriate format for your Tailwind version:

### Core Tailwind Variables

These standard Tailwind CSS variables are added with Genui's default color scheme:

```css
/* Light mode */
--background: /* White or appropriate light background */ --foreground:
  /* Dark text color */
  --primary: /* Genui brand primary color */
  --secondary: /* Secondary accent color */
  --muted: /* Muted backgrounds and borders */ --border: /* Border colors */
  /* ... additional standard Tailwind variables */;
```

### Genui-Specific Variables

These custom variables control Genui component layouts and behavior:
...
<Callout type="warn" title="Required Variables">
  The Genui-specific variables (`--panel-left-width`, `--panel-right-width`,
...
The default CSS file location depends on your framework: `app/globals.css` for Next.js, `src/index.css` for Vite.
...
@layer base {
  :root {
    /* Default Tailwind CSS Variables customized with genui colors */
    --background: 0 0% 100%;
    ...
    /* Genui Specific Variables needed for genui components */
    --radius: 0.5rem;
    --container: 210 29% 97%;
...
@theme inline {
  /* Tailwind CSS Variables customized with genui colors */
...
.dark {
  /* Dark Mode Tailwind CSS Variables customized with genui colors */
...
Ensure these Genui-specific variables are present:
...
1. **Keep your existing variables** that aren't listed in the Genui configuration
2. **Add missing Genui variables** from the appropriate version above
3. **Update conflicting variables** if you want to use Genui's color scheme
4. **Preserve your custom styling** outside of the CSS variables

### If you have existing `@layer base` blocks:

For Tailwind v3, add the Genui variables inside your existing `@layer base` block rather than creating a duplicate.
...
**Solution**: Ensure Genui layout variables (`--panel-left-width`, etc.) are defined and have appropriate values
...
<Callout type="info" title="Safe Configuration">
  ...
  configuration and only adds what's needed for Genui components to work.
</Callout>
