...
description: Step-by-step Genui CLI workflows for new projects, existing React apps, component management, and ongoing maintenance.
---

This guide covers the most common workflows you'll use with the Genui CLI, organized by scenario and use case.
...
<Callout type="info" title="API Key Required">
  ...
  `.env.local`: ``` NEXT_PUBLIC_GENUI_API_KEY=your_api_key_here ```
</Callout>
...
The Genui CLI automatically detects non-interactive environments (CI/CD, piped input/output) and provides helpful guidance. Use these patterns for automated pipelines.
...
RUN npx tambo init --api-key="$GENUI_API_KEY"
...
```bash
docker build --build-arg GENUI_API_KEY="$GENUI_API_KEY" .
```
