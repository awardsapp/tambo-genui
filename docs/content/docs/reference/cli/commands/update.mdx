---
title: update
description: Update specific or all installed Genui components to their latest versions and apply required CSS or Tailwind changes.
---

`npx genui update <componentname>` or `npx genui update installed`

Updates specific Genui components or all installed components to their latest versions from the registry.

**Examples:**

```bash
# Update specific components
npx genui update form graph

# Update ALL installed components at once
npx genui update installed

# Update with custom directory
npx genui update form --prefix=src/components/ui

# Skip confirmation prompts
npx genui update installed --yes
```
