...
- **Next.js:** `NEXT_PUBLIC_GENUI_API_KEY=your_api_key_here`
- **Expo:** `EXPO_PUBLIC_GENUI_API_KEY=your_api_key_here`
- **Vite:** `VITE_GENUI_API_KEY=your_api_key_here`
- **Other:** `GENUI_API_KEY=your_api_key_here`
...
Create a new Genui Cloud project and generate an API key. Requires authentication:
...
### Genui Cloud (Recommended)

**Setup time:** ~1 minute

The fastest way to get started. Genui Cloud handles all backend infrastructure for you.
...
? How would you like to host your Tambo project?
  ❯ Genui Cloud (~1 minute) [Recommended]
...
1. Clone the repository:
   git clone https://github.com/awardsapp/tambo-genui-core.git
...
4. Set this in your .env.local:
   NEXT_PUBLIC_GENUI_API_URL=http://localhost:3030
...
```bash
cat .env.local | grep GENUI_API_KEY
```
