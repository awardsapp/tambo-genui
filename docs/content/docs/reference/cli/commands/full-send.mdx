---
title: full-send
description: Run the `genui full-send` command to initialize your project, generate config, and install recommended Genui components in one step.
---
...
2. Creates the `lib/genui.ts` configuration file
...
npx genui full-send --yes

# Use legacy peer deps
npx genui full-send --legacy-peer-deps
```
...
3. Creates the `lib/genui.ts` configuration file
4. Registers all components in the configuration
5. Copies GenuiProvider setup code to your clipboard

<Callout type="info" title="Clipboard Integration">
  The setup code for GenuiProvider is automatically copied to your clipboard.
...
After running `full-send`, add the GenuiProvider to your app. The CLI detects your framework and shows the appropriate setup code.
...
import { MessageThreadFull } from "@/components/genui/message-thread-full";
...
For Expo projects, `full-send` skips web component installation since the registry components use DOM and Tailwind CSS. Use `@workspace/react` hooks directly to build your native UI.
...
export default function App() {
  return (
    <TamboProvider
      apiKey={process.env.EXPO_PUBLIC_GENUI_API_KEY ?? ""}
      components={components}
    >
...
@layer base {
  :root {
    ...
    /* Genui-specific variables for component layouts */
    --radius: 0.5rem;
    --container: 210 29% 97%;
...
:root {
  ...
  /* Genui-specific layout variables */
...
<Callout type="info" title="Automatic Configuration">
  ...
  configure these files unless you're customizing the default Genui styling.
</Callout>

<Callout type="warn" title="CSS Variable Requirements">
  Genui components require specific CSS variables to function properly. The
