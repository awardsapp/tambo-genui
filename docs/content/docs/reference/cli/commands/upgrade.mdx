---
title: upgrade
description: Upgrade your entire Genui project—including packages, components, and configuration—to known-safe versions with the `genui upgrade` command.
---

`npx genui upgrade`

Upgrades your entire genui project including npm packages, components, and AI coding assistant guidance to the latest versions.

**What it upgrades:**

1. **NPM Packages**: Updates known safe packages (UI components, utilities, and genui dependencies)
2. **Components**: Updates all installed genui components to latest versions
...
npx genui upgrade --yes

# Upgrade with custom component directory
npx genui upgrade --yes --prefix=src/components/ui

# Use legacy peer deps
npx genui upgrade --yes --legacy-peer-deps

# Skip agent docs updates
npx genui upgrade --yes --skip-agent-docs
```
...
<Callout type="info" title="Smart Package Filtering">
  ...
  utilities, and genui-specific packages. Core framework packages (like React,
  Next.js, TypeScript) are excluded to prevent breaking changes.
</Callout>

**Safe packages include:**

- genui packages (`@workspace/react`, `@workspace/typescript-sdk`)
...
The CLI will update your `globals.css` and `tailwind.config.ts` files to ensure they're compatible with the latest genui components and your current Tailwind CSS version.
...
If the upgrade command detects that your components are split across different locations (some in `components/genui/`, others in `components/ui/`), it will offer to migrate them:

```bash
⚠ Warning: Found components in both locations:
  • components/genui/: message-thread-full, control-bar
  • components/ui/: form, button

? Components should be in the same location. Would you like to:
  ❯ Migrate to components/genui/ (recommended)
...
- Genui components should live in `components/genui/` (current standard)
...
```bash
? Found legacy genui-ai.mdc cursor rules. Replace with AGENTS.md/CLAUDE.md? (Y/n)
...
- Removes the legacy `.cursor/rules/genui-ai.mdc` file if it was created by older Genui templates
...
- Creates or updates `AGENTS.md` and `CLAUDE.md` with current Genui guidance
