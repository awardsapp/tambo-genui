...
`ThreadDropdown` in `@workspace/react-ui-base` owns thread action behavior (switch, create) while keeping styling and popover/dropdown presentation in `@workspace/ui-registry` or your own UI layer.
