...
`McpPrompts` in `@workspace/react-ui-base` owns the MCP prompt selection lifecycle — listing available prompts, fetching selected prompt content, validating format, and surfacing errors — while keeping styling in `@workspace/ui-registry` or your own UI layer.
