...
`Elicitation` in `@workspace/react-ui-base` owns schema parsing, validation, and accept/decline/cancel behavior while exposing composable UI primitives.
...
| ------------ | ---------------------------------------------- | -------- | ------------------------------------------------ |
| `request`    | `GenuiElicitationRequest`                      | Required | Incoming elicitation request schema and message. |
| `onResponse` | `(response: GenuiElicitationResponse) => void` | Required | Callback for accept/decline/cancel responses.    |
