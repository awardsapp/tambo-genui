...
`MessageInput` in `@workspace/react-ui-base` owns message-authoring behavior while keeping styling in `@workspace/ui-registry` or your own UI layer.
...
| ---------- | -------------------------------------- | ----------- | -------------------------------- |
| `inputRef` | `React.RefObject<GenuiEditor \| null>` | `undefined` | Ref for editor-level operations. |
