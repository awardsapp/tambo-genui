...
`ThreadContent` in `@workspace/react-ui-base` owns thread timeline state derivation — messages, generation status, empty/loading detection — while keeping styling in `@workspace/ui-registry` or your own UI layer.
