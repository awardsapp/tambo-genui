...
`GenerationStage` in `@workspace/react-ui-base` reads thread generation state from `useTambo()` and provides context-driven visibility for waiting and streaming indicators.
