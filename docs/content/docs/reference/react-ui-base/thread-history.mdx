...
`ThreadHistory` in `@workspace/react-ui-base` owns thread collection state, search filtering, and selection behavior while keeping styling in `@workspace/ui-registry` or your own UI layer.
