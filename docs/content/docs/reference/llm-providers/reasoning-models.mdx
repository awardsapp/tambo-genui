...
<Callout type="info" title="Supported Providers">
  Genui currently supports reasoning capabilities for **OpenAI** and **Google
...
Genui components automatically handle reasoning display - no additional code required.

### Built-in Support

When you add Genui components from the CLI, reasoning support is included out-of-the-box:

```bash
npx genui add message
```
...
<Callout type="tip" title="Zero Configuration">
  If you're using Genui's pre-built components (message, thread-content,
...
- Monitor token usage in your Genui Cloud usage dashboard
