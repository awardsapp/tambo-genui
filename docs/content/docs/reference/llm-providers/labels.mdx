...
Models in genui carry a **status label**, shown when you select a model from the LLM settings  
(**Dashboard → Project → Settings → LLM Providers**).

### Why Use Labels?

- **Set expectations**: Understand genui’s confidence level for each model.
...
| Label          | Meaning                                                            |
| -------------- | ------------------------------------------------------------------ |
| `tested`       | Validated on common genui tasks. Recommended for most workflows.   |
...
- **[Anthropic Models](/reference/llm-providers/anthropic)** - Claude 4.5, 4.1, and 4 family models
...
You can change providers and models at the project level under **LLM Provider Settings**. genui will apply your token limits and defaults accordingly.
