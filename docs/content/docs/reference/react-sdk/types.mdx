...
description: TypeScript interfaces and types exported by @workspace/react
---

The `@workspace/react` package exports TypeScript types and interfaces for building type-safe AI applications.
...
Detailed streaming state returned by `useGenui()`.
...
Discriminated union tracking the SDK's authentication lifecycle. Returned by [`useGenui()`](/reference/react-sdk/hooks#usegenui) as `authState`.
...
function AuthAwareChat() {
  const { authState, isIdentified } = useGenui();
...
Configuration interface for the Genui provider. Passed to [`GenuiProvider`](/reference/react-sdk/providers#genuiprovider) or accessed via `useGenuiConfig()`.

```typescript
interface GenuiConfig {
  userKey?: string;
...
Message type for seeding new threads with initial messages. Re-exported from `@workspace/typescript-sdk`.
...
```typescript
interface GenuiComponentContent {
  type: "component";
  id: string;
  name: string;
...
A tool invocation with display metadata. Extends the base `ToolUseContent` from the SDK with computed properties populated by `useGenui()`.

```typescript
interface GenuiToolUseContent {
  type: "tool_use";
  id: string;
  name: string;
...
<Callout type="info">
  The `input` object has internal `_genui_*` properties automatically stripped.
  Display messages are available via `genuiDisplayProps`.
</Callout>
...
```typescript
interface GenuiToolDisplayProps {
...
Configuration for registering a component with Genui.
...
| `associatedTools`  | `GenuiTool[]`     | No       | Tools associated with this component                               |
...
### GenuiCurrentComponent

Component metadata returned by [`useGenuiCurrentComponent`](/reference/react-sdk/hooks#usegenuicurrentcomponent). Works for both AI-generated and interactable components.

```typescript
interface GenuiCurrentComponent {
...
The same `GenuiTool` interface from the base SDK is used.
...
```typescript
function isGenuiCustomEvent(event: {
  name?: string;
}): event is GenuiCustomEvent;
function asGenuiCustomEvent(event: CustomEvent): GenuiCustomEvent | undefined;
```
...
Options passed to `useGenuiThreadInput().submit()`.
...
Overall streaming status for a component, returned by `useGenuiStreamStatus()`.
...
Per-prop streaming status, returned by `useGenuiStreamStatus()`.
...
| Property              | Type      | Description                                                                                                                         |
| --------------------- | --------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| `genuiStreamableHint` | `boolean` | When `true`, the tool can be called with partially-streamed arguments as they arrive. Use for read-only tools without side effects. |

`ToolAnnotations` can be set on both `GenuiTool` and `GenuiComponent` via the `annotations` property.

## Interactable Types

### InteractableComponent

Represents a registered interactable component instance. Extends [`GenuiComponent`](#genuicomponent) with instance-specific state. The internal type name is `GenuiInteractableComponent`.

```typescript
interface GenuiInteractableComponent<
  Props = Record<string, unknown>,
  State = Record<string, unknown>,
> extends GenuiComponent {
  id: string;
  props: Props;
  isSelected?: boolean;
  state?: State;
...
Configuration passed to `withGenuiInteractable()` to make a component interactable.
...
| Property        | Type                     | Required | Description                                      |
| --------------- | ------------------------ | -------- | ------------------------------------------------ |
| `componentName` | `string`                 | Yes      | Name used for identification in Genui            |
...
### WithGenuiInteractableProps

Props injected by `withGenuiInteractable()` that can be passed to the wrapped component.

```typescript
interface WithGenuiInteractableProps {
  interactableId?: string;
...
| `onPropsUpdate`       | `(newProps: Record<string, unknown>) => void` | Called when Genui updates the component's props     |
...
- `UseGenuiReturn` - Return type of [`useGenui()`](/reference/react-sdk/hooks#usegenui)
- `GenuiConfig` - Provider configuration interface
- `GenuiThreadInputContextProps` - Context interface for [`useGenuiThreadInput()`](/reference/react-sdk/hooks#usegenuithreadinput)

### Component & Tool Types

- `GenuiComponent` - Base component registration interface
- `GenuiTool` - Tool definition interface
- `ComponentRegistry` - Map of registered components
- `ParameterSpec` - Parameter specification
- `InteractableComponent` - Alias for `GenuiInteractableComponent`
- `GenuiInteractableContext` - Full interactable registry context (see [`useGenuiInteractable()`](/reference/react-sdk/hooks#usegenuiinteractable))
