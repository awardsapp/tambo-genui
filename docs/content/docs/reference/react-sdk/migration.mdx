...
description: Step-by-step guide for migrating from @workspace/react (pre-1.0.0) to @workspace/react 1.0.
---

import { Tab, Tabs } from "fumadocs-ui/components/tabs";

This guide walks you through migrating from `@workspace/react` version 0.x to version 1.0. Follow the steps in order — each one builds on the previous.
...
Upgrade `@workspace/react` from version 0.x to version 1.0:

```bash
npm install @workspace/react@latest
```

Your imports stay the same — both versions use `@workspace/react`:
...
The `GenuiProvider` API is mostly the same, but `contextKey` is replaced by `userKey`:
...
The `useGenui()` return shape is different:

```tsx title="Before"
function Chat() {
  const { thread, messages, generationStage, isStreaming } = useGenui();
...
| `sendThreadMessage()`    | _removed_                           | Use `useGenuiThreadInput().submit()` instead                                          |
...
```tsx title="Before"
function Message({ message }: { message: GenuiThreadMessage }) {
  return (
    <div>
...
```tsx title="After"
import { ComponentRenderer, type GenuiThreadMessage } from "@workspace/react";
...
| `tool_use`    | A tool call. Includes `name`, `input` (cleaned of internal `_genui_*` props), `hasCompleted`, and `statusMessage`. |
...
<Callout type="info" title="Rendered Components on Content Blocks">
  For convenience, `useGenui()` pre-renders component content blocks and
...
The `useGenuiComponentState` API is the same tuple pattern, but with an added `flush` function:
...
```tsx title="After"
const [state, setState, { isPending, error, flush }] = useGenuiComponentState(
  "myStateKey",
  initialValue,
...
`useGenuiComponentState` only works inside components rendered via `ComponentRenderer` — the renderer provides the necessary context.

## Step 7: Update stream status

The `useGenuiStreamStatus` return shape now includes additional flags:

```tsx title="Before"
const { isPending, isStreaming, propStatus } = useGenuiStreamStatus();
```
...
The `useGenuiThreadList` return value structure has changed:

```tsx title="Before"
const { data: threads, isLoading } = useGenuiThreadList();
```
...
The `useGenuiSuggestions` hook now includes additional control functions:

```tsx title="Before"
const { suggestions, isLoading } = useGenuiSuggestions();
```
...
The `GenuiStubProvider` API is more explicit about what state it provides:
...
`GenuiStubProvider` provides all contexts without making API calls, and requires explicit props for all state.
...
The HOC was renamed from `withInteractable` to `withGenuiInteractable` for consistency with the SDK naming convention. Usage is otherwise unchanged:
...
The `GenuiContextAttachmentProvider` is included in the provider hierarchy, and the `useGenuiContextAttachment` hook is available from the main package:
...
| Pre-1.0.0                         | 1.0                                   |
| --------------------------------- | ------------------------------------- |
| `GenuiProvider`                   | `GenuiProvider`                       |
| `GenuiStubProvider`               | `GenuiStubProvider`                   |
| `useGenui()`                      | `useGenui()`                          |
| `useGenuiThreadInput()`           | `useGenuiThreadInput()`               |
| `useGenuiComponentState()`        | `useGenuiComponentState()`            |
| `useGenuiStreamStatus()`          | `useGenuiStreamStatus()`              |
| `useGenuiThreadList()`            | `useGenuiThreadList()`                |
| `useGenuiSuggestions()`           | `useGenuiSuggestions()`               |
| `GenuiThreadMessage`              | `GenuiThreadMessage`                  |
| `GenerationStage`                 | `streamingState.status` (`RunStatus`) |
| `withInteractable`                | `withGenuiInteractable`               |
| `useGenuiInteractable`            | `useGenuiInteractable`                |
...
```typescript
import { z } from "zod";
import { useGenui, defineTool } from "@workspace/react";
