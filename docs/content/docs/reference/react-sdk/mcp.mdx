...
description: API reference for @workspace/react/mcp — hooks, components, utilities, and types for MCP server integration.
---

The `@workspace/react/mcp` subpath provides hooks, a provider component, utilities, and types for integrating MCP servers into your Genui application. All exports are imported from `@workspace/react/mcp`.
...
Provider that manages MCP server connections and tool registration. Must be placed inside `GenuiProvider`. MCP servers are configured via the `mcpServers` prop on `GenuiProvider`, not on this provider.
...
```tsx
import { useGenuiMcpServers } from "@workspace/react/mcp";
...
```tsx
import { useGenuiMcpElicitation } from "@workspace/react/mcp";
...
| -------------------- | -------------------------------------------------------- | ---------------------------------------------- |
| `elicitation`        | `GenuiElicitationRequest \| null`                        | Current elicitation request, or null if none   |
| `resolveElicitation` | `((response: GenuiElicitationResponse) => void) \| null` | Function to respond to the elicitation request |
...
```tsx
import { useGenuiMcpPromptList } from "@workspace/react/mcp";

function PromptPicker() {
  const { data, isPending } = useGenuiMcpPromptList();
...
function PromptView({ promptName }: { promptName: string }) {
  const { data, isPending } = useGenuiMcpPrompt(promptName, {
    projectId: "abc-123",
  });
...
function ResourceBrowser() {
  const { data, isPending } = useGenuiMcpResourceList("search term");
...
```tsx
import { useGenuiMcpResource } from "@workspace/react/mcp";

function ResourceViewer({ uri }: { uri: string }) {
  const { data, isPending } = useGenuiMcpResource(uri);
...
Configuration object for an MCP server, passed via the `mcpServers` prop on `GenuiProvider`.
...
Elicitation request surfaced by `useGenuiMcpElicitation`.
...
A prompt from an MCP server, as returned by `useGenuiMcpPromptList`.
...
A resource from an MCP server or the local registry, as returned by `useGenuiMcpResourceList`.
...
Configuration object accepted by `GenuiProvider`'s `mcpServers` prop. Also exported from `@workspace/react/mcp`.
