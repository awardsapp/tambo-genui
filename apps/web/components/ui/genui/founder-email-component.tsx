...
export const FounderEmailComponent = ({
  ...
  const [subject, setSubject] = useGenuiComponentState(
    "emailSubject",
    aiGeneratedSubject || "",
  );
  const [body, setBody] = useGenuiComponentState(
    "emailBody",
    aiGeneratedBody || "",
  );
  const [usersEmail, setUsersEmail] = useGenuiComponentState(
    "usersEmail",
    aiGeneratedUsersEmail || "",
  );
  const [isSent, setIsSent] = useGenuiComponentState("isSent", false);
  const [isLoading, setIsLoading] = useGenuiComponentState("isLoading", false);
  const [error, setError] = useGenuiComponentState<string | null>(
    "error",
    null,
  );
