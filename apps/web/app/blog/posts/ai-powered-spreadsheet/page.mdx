---
title: "Building an AI-Powered Spreadsheet with Genui"
date: "2025-10-28"
...
I've been wanting to build an AI-powered spreadsheet for a while now, and honestly, Google's Gemini integration for Sheets hasn't impressed me yet. So I decided to see what we could do with [Genui](https://github.com/genui-ai/genui) (please give us a star if you find it useful!).
...
Try it yourself: [cheatsheet.genui.co](https://cheatsheet.genui.co)

The timing worked out perfectly. I'd been looking for a good excuse to show off some of Genui's more advanced patterns, and when I found ReactGrid (this solid open-source spreadsheet library), I knew I had to build something with it. The whole project is completely open source and built with Genui (a React SDK for adding a natural language interface to an application) and ReactGrid, so if you want to see how it works or build something similar, [the code is all there](https://github.com/michaelmagan/cheatsheet).
...
Tabs are just lightweight metadata: an ID and a name. The standard [interactable pattern](https://docs.genui.co/concepts/generative-interfaces/interactable-components) works great for this.
...
This is way lighter than the full spreadsheet data, so it's fine to just emit it as structured metadata. The AI can see tab names, rename them, switch between them, whatever. And because it's using the standard interactable pattern, it all just works with Genui's built-in machinery.
...
Genui's [context helper feature](https://docs.genui.co/concepts/additional-context) makes this super easy. I just write a function that formats the data, register it, and it automatically gets included with every message. I don't have to think about when or how to inject that context. It's just always there.
...
Pretty simple. The spreadsheet component calls `updateSpreadsheetSelection()` whenever the selection changes, and Genui's context helper system picks it up automatically.
