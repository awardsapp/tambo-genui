...
1. **Wrapped our existing React components in Zod schemas**. This gave genui a contract for what props each component needed and what they did.

2. **Registered those components with genui**. Now the assistant had a library of real UI it could choose from, instead of spitting out text instructions.
...
5. **Add Chat Assistant**. We then pass the registry of information and the MCP server to the GenuiProvider. This is where the magic happens.

That’s the mental model: schemas for shape, components for UI, tools/docs for context. Everything else is just configuration.

React Components -> Zod Schemas -> Tools/MCP Servers -> GenuiProvider -> Chat Assistant

## How we did it

By the way our repo is [open source](https://github.com/genui-ai/genui), so you can see the code for yourself.
...
Example: the [APIKeyList component](https://github.com/genui-ai/genui/blob/main/apps/web/components/dashboard-components/project-details/api-key-list.tsx#L26).
...
Example: Registering the [APIKeyList component](https://github.com/genui-ai/genui/blob/main/apps/web/components/ui/genui/chatwithgenui/config.ts#L62).
...
This is where your backend actually hooks in. In our case, we wired in the [`generateApiKey` endpoint](https://github.com/genui-ai/genui/blob/main/apps/web/components/ui/genui/chatwithgenui/tools.ts#L162). You'd do the same with your own API.
...
export function useGenuiManagementTools() {
...
### 4. Configure the GenuiProvider

That’s it--you're done.  
_(Note: `useGenuiManagementTools` uses `registerTool` internally, so you don’t need to pass it into the provider manually.)_
...
We also wanted the assistant to explain genui itself. That’s where Inkeep’s MCP server came in. It adds doc context into the assistant so it can answer questions, drop in the right code snippet, or even render UI to set things up. To add one:
...
You can plug in your own MCP server to give the assistant context about your product. Our [MCP server](https://mcp.genui.co/mcp) is also public if you're building with genui and want doc-aware context.
...
![Genui AI assistant chat interface showing interactive React components rendered within the conversation](/blog/arrow-to-genui.png)

## Wrapping up

We think AI should leverage the power of language and graphical user interfaces. With genui and MCP, we ended up with an assistant that can both explain things with natural language and can perform actions and display rich information without leaving the chat. We think this is a powerful paradigm for building AI-powered applications.

Check out our [repository](https://github.com/genui-ai/genui) or join our [discord](https://genui.co/discord). We're always around to help!
