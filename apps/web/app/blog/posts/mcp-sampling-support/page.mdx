---
title: "Apps using Genui now support MCP Sampling"
date: "2025-10-22"
...
[Genui](https://genui.co) helps developers build LLM-based AI web apps by discovering, understanding, and packaging up emerging UX patterns so you can integrate them without building from scratch.
...
At Genui, we're building comprehensive MCP support so your app becomes "MCP-enabled" automatically. Today we're announcing support for [MCP Sampling](https://modelcontextprotocol.io/docs/learn/client-concepts#sampling), following our previous release of [MCP tools](https://modelcontextprotocol.io/docs/learn/server-concepts#tools) support.
...
With MCP Sampling through Genui, the server uses Genui's AI setup.
...
## The UX of Sampling in Genui

Genui apps using our pre-built `Message` component display an expandable dropdown when Genui decides to use a tool. This dropdown shows details of the tool call to the user. We consider sampling requests as "sub-conversations" between the MCP tool and Genui, so Genui represents them as expandable lists of messages within the tool's information dropdown.

The following example uses an `emojify_via_llm` MCP tool which uses Sampling to turn a string into emojis. Within the dropdown you can see the messages the tool sent to Genui and the responses it received.
...
## How to customize Genui's Sampling UI

Behind the scenes, Genui adds messages sent and returned through sampling requests to the user's conversation thread. The sampling messages have a `parentMessageId` field pointing to the assistant message tool call that triggered the sampling request. This is what distinguishes them as "sub messages" of a normal conversation message. Note that Genui passes a `_meta` field containing the `parentMessageId` to MCP tools and expects sampling requests to pass that along to Genui.

To build your own custom sampling UI, use Genui’s [React SDK](https://github.com/genui-ai/genui) to get the list of messages for the current thread, and use the `parentMessageId` field on the messages to group and show sampling messages in any way you prefer.
...
## How to enable MCP Sampling in your app using Genui

If you already have an app with Genui running, you’ll need to:

- Upgrade your `genui-ai/react` dependency to at least version `0.57.0` using `npm install @workspace/react@0.57.0`
- Upgrade your installed Genui `Message` component using `npx genui@latest upgrade message`

If you haven’t integrated Genui into your web app yet, new installations will automatically contain MCP Sampling functionality and UI. To get started, check out our [documentation](https://docs.genui.co/getting-started/quickstart).
