...
import { MessageInputTextareaWithInteractables } from "@/components/ui/genui/message-input-with-interactables";
...
export function GenuiSubscribeIntegration() {
  const { registerComponent, messages } = useGenui();
  const { setValue, submit } = useGenuiThreadInput();
  ...
  return (
    <>
      <Dialog open={showWelcomeDialog} onOpenChange={handleWelcomeDialogClose}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Welcome to Genui Subscribe!</DialogTitle>
            <DialogDescription>
