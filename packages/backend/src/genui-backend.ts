...
/** The current model options for the GenuiBackend, filled in with defaults */
...
export async function createGenuiBackend(
  apiKey: string | undefined,
  chainId: string,
  userId: string,
  options: GenuiBackendOptions = { aiProviderType: AiProviderType.LLM },
): Promise<GenuiBackend> {
  return await AgenticGenuiBackend.create(apiKey, chainId, userId, options);
}

class AgenticGenuiBackend implements GenuiBackend {
  llmClient: LLMClient;
  /** The current model options for the GenuiBackend, filled in with defaults */
  ...
  static async create(
    ...
    switch (aiProviderType) {
      case AiProviderType.LLM: {
        return new AgenticGenuiBackend(modelOptions, llmClient);
      }
      case AiProviderType.AGENT: {
        ...
        return new AgenticGenuiBackend(modelOptions, llmClient, agentClient);
      }
      default:
