...
jest.mock("@workspace/typescript-sdk", () => {
  ...
  function MockGenuiAI(this: unknown, opts: unknown) {
  ...
  return { __esModule: true, default: MockGenuiAI };
});
...
jest.mock("./genui-stream", () => ({
  GenuiStream: function MockGenuiStream() {
...
function createMockTool(name: string): GenuiTool {
  return {
    name,
...
describe("GenuiClient", () => {
  describe("constructor", () => {
    it("creates client with minimal options", () => {
      const client = new GenuiClient({ apiKey: "test-key" });
      expect(client).toBeInstanceOf(GenuiClient);
    });

    it("throws when both genuiUrl and environment are specified", () => {
      expect(
        () =>
          new GenuiClient({
            apiKey: "test-key",
            genuiUrl: "https://custom.example.com",
            environment: "production",
          }),
      ).toThrow(
        "Cannot specify both 'genuiUrl' and 'environment'. Choose one.",
      );
    });
    ...
    it("registers initial tools from options", () => {
      ...
      const client = new GenuiClient({ apiKey: "test-key", tools: [tool] });
      expect(client).toBeInstanceOf(GenuiClient);
    });

    it("accepts genuiUrl option without environment", () => {
      const client = new GenuiClient({
        apiKey: "test-key",
        genuiUrl: "https://custom.example.com",
      });
      expect(client).toBeInstanceOf(GenuiClient);
    ...
    it("accepts environment option without genuiUrl", () => {
      const client = new GenuiClient({
        apiKey: "test-key",
        environment: "staging",
      });
      expect(client).toBeInstanceOf(GenuiClient);
      expect(mocks.sdkConstructorArgs).toEqual(
        expect.objectContaining({ baseURL: "https://api.staging.genui.co" }),
      );
    });

    it("uses production URL by default", () => {
      new GenuiClient({ apiKey: "test-key" });
      expect(mocks.sdkConstructorArgs).toEqual(
        expect.objectContaining({ baseURL: "https://api.genui.co" }),
      );
    });

    it("passes apiKey to SDK client", () => {
      new GenuiClient({ apiKey: "my-api-key" });
      expect(mocks.sdkConstructorArgs).toEqual(
