...
import type { GenuiStreamOptions, StreamEvent } from "./genui-stream";
import { GenuiStream } from "./genui-stream";
...
function makeFakeThread(threadId: string): GenuiThread {
...
function makeBaseOptions(
  overrides: Partial<GenuiStreamOptions> = {},
): GenuiStreamOptions {
  const fakeThread = makeFakeThread(overrides.threadId ?? "thread_123");
  return {
    client: {} as GenuiStreamOptions["client"],
    message: {
      content: [{ type: "text", text: "hello" }],
    } as GenuiStreamOptions["message"],
    threadId: "thread_123",
    componentList: {},
    toolRegistry: {},
    userKey: undefined,
...
describe("GenuiStream", () => {
  ...
  describe(".thread promise", () => {
    it("resolves to the final GenuiThread when stream completes", async () => {
      const events = makeEvents();
      setupStreamMocks(events);
      const stream = new GenuiStream(makeBaseOptions());
  ...
  describe("single consumption", () => {
    it("throws when iterated a second time", async () => {
      ...
      }).toThrow("GenuiStream can only be iterated once");
    });
  });
  ...
  describe("dispatch integration", () => {
    it("calls dispatch for each event with EVENT type and threadId", async () => {
      ...
      const stream = new GenuiStream(options);
