/**
 * GenuiStream - Streaming response handler for Genui AI
 *
...
import type { GenuiThread } from "./types/thread";
import { asGenuiCustomEvent, type RunAwaitingInputEvent } from "./types/event";
...
/**
 * Event yielded by the GenuiStream async iterator.
 */
...
/**
 * Options for creating a GenuiStream.
 */
export interface GenuiStreamOptions {
  /** The Genui API client. */
  client: GenuiAI;
  /** The message to send. */
  ...
  getThreadSnapshot: (threadId: string) => GenuiThread | undefined;
}
...
/**
 * GenuiStream handles streaming AI responses with tool execution.
 *
 ...
 */
export class GenuiStream implements AsyncIterable<StreamEvent> {
  ...
  private resolveThread!: (thread: GenuiThread) => void;
  ...
  /**
   * Create a new GenuiStream.
   * @param options - Configuration for the stream.
   */
  constructor(private readonly options: GenuiStreamOptions) {
    ...
    this.thread = new Promise<GenuiThread>((resolve, reject) => {
  ...
  [Symbol.asyncIterator](): AsyncIterableIterator<StreamEvent> {
    if (this.consumed) {
      throw new Error("GenuiStream can only be iterated once");
    }
    this.consumed = true;
      ...
      while (true) {
        ...
        for await (const event of handleEventStream(currentStream, { debug })) {
          ...
          if (event.type === EventType.CUSTOM) {
            ...
            if (customEvent?.name === "genui.run.awaiting_input") {
        ...
        if (stepCount >= maxSteps) {
          console.warn(
            `[GenuiStream] maxSteps (${maxSteps}) reached. Stream resolving with pending tool calls.`,
          );
          break;
        }
