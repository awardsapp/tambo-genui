...
import { GenuiMessageProvider } from "../v1/hooks/use-genui-current-message";
import type { GenuiThreadMessage } from "../v1/types/message";
...
> {
  /**
   * The name of the component, used for identification in Genui.
   */
   ...
   * (props update and state update). By default, `genuiStreamableHint` is `true`
   * so props/state updates stream in real-time. Set `{ genuiStreamableHint: false }`
...
/**
 * Props injected by withGenuiInteractable HOC. These can be passed to the wrapped
 * component to customize interactable behavior.
 */
export interface WithGenuiInteractableProps {
  /**
  ...
  /**
   * Callback fired when the component's serializable props are updated by Genui
...
/**
 * Higher-Order Component that makes any component interactable by genui.
 ...
 *   const [isPinned, setIsPinned] = useGenuiComponentState("isPinned", false);
 *   return (
 ...
 * };
 *
 * const MyInteractableNote = withGenuiInteractable(MyNote, {
 ...
 * ```
 */
export function withGenuiInteractable<ComponentProps extends object>(
  ...
  const GenuiInteractableWrapper: React.FC<
    ComponentProps & WithGenuiInteractableProps
  > = (props) => {
    const {
    ...
    // This allows useGenuiCurrentComponent to work with standalone interactable components
    const minimalMessage: GenuiThreadMessage = {
      id: interactableId,
    ...
    // Wrap with GenuiMessageProvider and ComponentContentProvider including interactable metadata
    return (
  ...
  GenuiInteractableWrapper.displayName = `withGenuiInteractable(${displayName})`;

  return GenuiInteractableWrapper;
}
