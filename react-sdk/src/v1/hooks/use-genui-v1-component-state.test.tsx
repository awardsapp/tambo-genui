...
import { useGenuiComponentState } from "./use-genui-v1-component-state";
...
jest.mock("../providers/genui-v1-stream-context", () => ({
...
import type { StreamState } from "@workspace/client";
import type { GenuiComponentContent } from "../types/message";

describe("useGenuiComponentState", () => {
  ...
  beforeEach(() => {
    ...
    // Setup default mocks
    jest.mocked(useGenuiClient).mockReturnValue({
      threads: {
        state: {
          updateState: mockUpdateState,
        },
      },
    } as unknown as ReturnType<typeof useGenuiClient>);
  ...
  describe("Initial State Management", () => {
    it("should initialize with initialValue when no server state exists", () => {
      ...
      const { result } = renderHook(() =>
        useGenuiComponentState("testKey", initialValue),
      );
    ...
    it("should handle undefined initialValue gracefully", () => {
      ...
      const { result } = renderHook(() => useGenuiComponentState("testKey"));
    ...
    it("should handle different data types correctly", () => {
      ...
      testCases.forEach(({ value }) => {
        ...
        const { result } = renderHook(() =>
          useGenuiComponentState("testKey", value),
        );
  ...
  describe("State Updates", () => {
    ...
    it("should support functional updates", () => {
      ...
      const { result } = renderHook(() =>
        useGenuiComponentState("counter", initialValue),
      );
    ...
    it("should work with complex objects", () => {
      ...
      const { result } = renderHook(() =>
        useGenuiComponentState("data", initialValue),
      );
  ...
  describe("Metadata", () => {
    it("should return isPending and error in meta", () => {
      ...
      const { result } = renderHook(() =>
        useGenuiComponentState("testKey", "initial"),
      );
  ...
  describe("Debouncing Behavior", () => {
    it("should use default debounce time of 500ms", () => {
      ...
      renderHook(() => useGenuiComponentState("testKey", "initial"));
    ...
    it("should use custom debounce time when provided", () => {
      ...
      renderHook(() =>
        useGenuiComponentState("testKey", "initial", customDebounceTime),
      );
  ...
  describe("Server State Sync", () => {
    ...
    it("should handle component not found in stream state", () => {
      ...
      const { result } = renderHook(() =>
        useGenuiComponentState("testKey", "default"),
      );
  ...
  describe("Interactable Component Support", () => {
    ...
    it("should initialize with initialValue when no interactable state exists", () => {
      ...
      const { result } = renderHook(() => useGenuiComponentState("count", 42));
    ...
    it("should initialize from interactable state when it exists", () => {
      ...
      const { result } = renderHook(() => useGenuiComponentState("count", 0));
    ...
    it("should sync from interactable provider when state changes externally", () => {
      ...
      const { result, rerender } = renderHook(() =>
        useGenuiComponentState("count", 0),
      );
    ...
    it("should set initial value in interactable state on mount when no existing state", () => {
      ...
      renderHook(() => useGenuiComponentState("count", 42));
    ...
    it("should not flush on unmount for interactable components", () => {
      ...
      const { unmount } = renderHook(() => useGenuiComponentState("count", 0));
