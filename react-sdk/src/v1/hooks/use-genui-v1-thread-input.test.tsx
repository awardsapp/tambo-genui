...
import { useGenuiSendMessage } from "./use-genui-v1-send-message";
import type { StreamAction, StreamState } from "@workspace/client";

// Mock useGenuiSendMessage
jest.mock("./use-genui-v1-send-message", () => ({
  useGenuiSendMessage: jest.fn(),
}));

// Mock useGenuiQueryClient to avoid GenuiClientProvider dependency
jest.mock("../../providers/genui-client-provider", () => ({
  useGenuiQueryClient: jest.fn(() => new QueryClient()),
...
describe("useGenuiThreadInput", () => {
    ...
    return function Wrapper({ children }: { children: React.ReactNode }) {
      ...
      return (
        <QueryClientProvider client={queryClient}>
          <GenuiStreamProvider {...streamProviderProps}>
            <GenuiThreadInputProvider>{children}</GenuiThreadInputProvider>
          </GenuiStreamProvider>
        </QueryClientProvider>
      );
    };
  ...
  beforeEach(() => {
    ...
    jest.mocked(useGenuiSendMessage).mockReturnValue({
  ...
  describe("State Management", () => {
    it("initializes with empty value", () => {
      const { result } = renderHook(() => useGenuiThreadInput(), {
  ...
  describe("Thread ID Management", () => {
    ...
    it("uses currentThreadId from stream state", () => {
      ...
      expect(
        jest.mocked(useGenuiSendMessage).mock.calls.map((call) => call[0]),
  ...
  describe("Error handling", () => {
    it("throws error when used outside provider", () => {
      ...
      expect(() => {
        renderHook(() => useGenuiThreadInput());
      }).toThrow(
  ...
  describe("Thread Isolation", () => {
    ...
    it("isolates pending state per thread when switching threads", async () => {
      ...
      const { result, rerender } = renderHook(() => useGenuiThreadInput(), {
        wrapper: Wrapper,
      });
