...
import {
  createRunStream,
  useGenuiSendMessage,
} from "./use-genui-v1-send-message";
...
jest.mock("../../providers/genui-context-helpers-provider", () => ({
  useGenuiContextHelpers: () => ({
    getAdditionalContext: jest.fn().mockResolvedValue([]),
  }),
}));

describe("useGenuiSendMessage", () => {
  ...
  function TestWrapper({ children }: { children: React.ReactNode }) {
    return (
      <QueryClientProvider client={queryClient}>
        <GenuiRegistryContext.Provider value={mockRegistry as any}>
  ...
  it("returns a mutation object", () => {
    const { result } = renderHook(() => useGenuiSendMessage("thread_123"), {
      wrapper: TestWrapper,
    });
  ...
  it("returns a mutation object when threadId is not provided", () => {
    const { result } = renderHook(() => useGenuiSendMessage(), {
      wrapper: TestWrapper,
    });
...
describe("useGenuiSendMessage mutation", () => {
  const mockThreadsRunsApi = {
    run: jest.fn(),
    create: jest.fn(),
  };
  ...
  it("works with default registry context when no provider is present", () => {
    // GenuiRegistryContext has a default value, so the hook should work
  ...
  it("uses lastCompletedRunId as previousRunId when streaming.runId is absent", async () => {
    ...
    const stateWithLastRun: import("@workspace/client").StreamState = {
    ...
    function TestWrapper({ children }: { children: React.ReactNode }) {
      return (
        <QueryClientProvider client={queryClient}>
          <GenuiRegistryContext.Provider value={mockRegistry as any}>
            <GenuiStreamProvider state={stateWithLastRun} dispatch={jest.fn()}>
              {children}
            </GenuiStreamProvider>
          </GenuiRegistryContext.Provider>
        </QueryClientProvider>
  ...
  it("logs error on mutation failure", async () => {
    ...
    await waitFor(() => {
      expect(consoleSpy).toHaveBeenCalledWith(
        "[useGenuiSendMessage] Mutation failed:",
        testError,
      );
    });
...
describe("useGenuiSendMessage auto thread name generation", () => {
  ...
  it("calls generateName when message count reaches threshold", async () => {
    ...
    jest.mocked(useGenuiConfig).mockReturnValue({
      userKey: undefined,
  ...
  it("logs error when generateName fails (does not throw)", async () => {
    ...
    await waitFor(() => {
      expect(consoleSpy).toHaveBeenCalledWith(
        "[useGenuiSendMessage] Failed to auto-generate thread name:",
        expect.any(Error),
      );
    });
...
describe("useGenuiSendMessage genuiStreamableHint", () => {
  const mockThreadsRunsApi = {
    run: jest.fn(),
    create: jest.fn(),
  };
  ...
  it("calls streamable tool on TOOL_CALL_ARGS events during streaming", async () => {
    ...
    const streamableTool: GenuiTool = {
      name: "write_story",
    ...
    const mockRegistry = {
      componentList: new Map(),
      toolRegistry: { write_story: streamableTool } as Record<
        string,
        GenuiTool
      >,
    };
    ...
    const firstStreamingCall = streamableToolFn.mock.calls[0][0] as Record<
      string,
      unknown
    >;
  ...
  it("does NOT call non-streamable tool during TOOL_CALL_ARGS events", async () => {
    ...
    const normalTool: GenuiTool = {
      name: "normal_tool",
      description: "Normal tool (not streamable)",
      tool: normalToolFn,
      inputSchema: z.object({ query: z.string() }),
      outputSchema: z.string(),
      // No genuiStreamableHint
    };

    const mockRegistry = {
      componentList: new Map(),
      toolRegistry: { normal_tool: normalTool } as Record<string, GenuiTool>,
    };
