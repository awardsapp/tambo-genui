...
/**
 * useGenuiStreamStatus - Stream Status Hook
 *
...
function usePropsStreamingStatus<Props extends object>(
  ...
  componentStreamingState: GenuiComponentContent["streamingState"] | undefined,
...
/**
 * Track streaming status for Genui component props.
 *
 ...
 *
 * Pair with `useGenuiComponentState` to disable inputs while streaming.
 * @see {@link https://docs.genui.co/concepts/generative-interfaces/component-state}
 ...
 * ```
 */
export function useGenuiStreamStatus<
...
>(): {
  ...
  useEffect(() => {
    if (componentId !== initialComponentIdRef.current) {
      console.error(
        `useGenuiStreamStatus: componentId changed from "${initialComponentIdRef.current}" to "${componentId}". ` +
