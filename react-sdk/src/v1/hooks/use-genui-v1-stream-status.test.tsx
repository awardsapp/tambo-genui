...
import { useGenuiStreamStatus } from "./use-genui-v1-stream-status";
...
import type { StreamState, ThreadState } from "@workspace/client";
import type {
...
const mockUseGenuiComponentContent = jest.mocked(useComponentContent);
const mockUseStreamState = jest.mocked(useStreamState);

/**
 * Helper to create a component content block.
 * @returns A GenuiComponentContent with test defaults.
 */
function createComponentContent(
  overrides: Partial<GenuiComponentContent> = {},
): GenuiComponentContent {
  return {
    type: "component",
    id: "test-component",
 ...
 * @returns A GenuiThreadMessage with a component content block.
 */
function createMessage(
  componentContent: GenuiComponentContent,
...
describe("useGenuiStreamStatus", () => {
  beforeEach(() => {
    // Restore window for client-side tests
    global.window = originalWindow;

    // Default mock implementations
    mockUseGenuiComponentContent.mockReturnValue({
  ...
  describe("Initial State", () => {
    it("should start with all flags as pending when component streaming is 'started' and no props", () => {
      ...
      const { result } = renderHook(() =>
        useGenuiStreamStatus<{ title: string; body: string }>(),
      );
  ...
  describe("Type Safety", () => {
    it("should provide strongly typed prop status based on generic", () => {
      ...
      const { result } = renderHook(() => useGenuiStreamStatus<TestProps>());
    ...
    it("should work without generic type parameter", () => {
      ...
      const { result } = renderHook(() => useGenuiStreamStatus());
  ...
  describe("Edge Cases", () => {
    ...
    it("should error when component ID changes unexpectedly", () => {
      ...
      const { rerender } = renderHook(() =>
        useGenuiStreamStatus<{ title: string }>(),
      );
