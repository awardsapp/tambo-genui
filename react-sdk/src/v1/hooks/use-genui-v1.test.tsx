...
import { GenuiStreamProvider } from "../providers/genui-v1-stream-context";
import {
  streamReducer,
  type StreamState,
  type StreamAction,
...
import { useGenui } from "./use-genui-v1";
...
jest.mock("../providers/genui-v1-provider", () => {
  const actual = jest.requireActual("../providers/genui-v1-provider");
  return {
    ...actual,
...
describe("useGenui", () => {
  ...
  const mockRegistry: GenuiRegistryContextType = {
    componentList: {},
    toolRegistry: {},
  ...
  function TestWrapper({ children }: { children: React.ReactNode }) {
    return (
      <QueryClientProvider client={queryClient}>
        <GenuiRegistryContext.Provider value={mockRegistry}>
  ...
  function createWrapperWithState(
    state: StreamState,
    registry: GenuiRegistryContextType = mockRegistry,
  ) {
      ...
      return (
        <QueryClientProvider client={queryClient}>
          <GenuiRegistryContext.Provider value={registry}>
            <GenuiStreamProvider state={state} dispatch={noopDispatch}>
              {children}
            </GenuiStreamProvider>
          </GenuiRegistryContext.Provider>
        </QueryClientProvider>
  ...
  function createWrapperWithRealReducer(
    ...
    return function WrapperWithRealReducer({
      ...
      return (
        <QueryClientProvider client={queryClient}>
          <GenuiRegistryContext.Provider value={mockRegistry}>
            <GenuiStreamProvider state={state} dispatch={dispatch}>
              {children}
            </GenuiStreamProvider>
          </GenuiRegistryContext.Provider>
        </QueryClientProvider>
  ...
  beforeEach(() => {
    ...
    jest.mocked(useGenuiClient).mockReturnValue(mockGenuiClient);
    jest.mocked(useGenuiQueryClient).mockReturnValue(queryClient);
    jest.clearAllMocks();
    delete (mockGenuiClient.threads as any).update;
  });

  it("returns client from useGenuiClient", () => {
    const { result } = renderHook(() => useGenui(), {
      wrapper: TestWrapper,
    });

    expect(result.current.client).toBe(mockGenuiClient);
  });
  ...
  describe("component content transformation", () => {
    ...
    it("adds renderedComponent to component content blocks", () => {
      ...
      expect(
        (content as GenuiComponentContent).renderedComponent,
      ).toBeDefined();
    });
    ...
    it("preserves non-component content blocks unchanged", () => {
      ...
      expect(
        (componentContent as GenuiComponentContent).renderedComponent,
      ).toBeDefined();
    });
    ...
    it("caches rendered components and returns same reference when props unchanged", () => {
      ...
      const { result, rerender } = renderHook(() => useGenui(), {
        wrapper: createWrapperWithState(state),
      });

      const firstRender = (
        result.current.messages[0].content[0] as GenuiComponentContent
      ).renderedComponent;
  ...
  describe("tool_use content transformation", () => {
    it("adds computed properties to tool_use content", () => {
      ...
      const content = result.current.messages[0]
        .content[0] as GenuiToolUseContent;
      ...
      expect(content.genuiDisplayProps).toBeDefined();
    });
    ...
    it("uses _genui_statusMessage when tool is in progress", () => {
      const state: StreamState = {
        threadMap: {
          thread_123: {
            thread: {
                  ...
                  content: [
                    {
                      ...
                      input: {
                        location: "NYC",
                        _genui_statusMessage: "Fetching weather for NYC...",
                      },
                    },
                  ],
                  createdAt: new Date().toISOString(),
      ...
      expect(content.genuiDisplayProps?._genui_statusMessage).toBe(
        "Fetching weather for NYC...",
      );
    });

    it("uses _genui_completionStatusMessage when tool is completed", () => {
      const state: StreamState = {
        threadMap: {
          thread_123: {
            thread: {
                  ...
                  content: [
                    {
                      ...
                      input: {
                        location: "NYC",
                        _genui_statusMessage: "Fetching weather...",
                        _genui_completionStatusMessage: "Got weather for NYC",
                      },
                    },
                  ],
                  createdAt: new Date().toISOString(),
    ...
    it("filters _genui_* properties from input", () => {
      const state: StreamState = {
        threadMap: {
          thread_123: {
            thread: {
                  ...
                  content: [
                    {
                      ...
                      input: {
                        ...
                        _genui_completionStatusMessage: "Done",
                      },
                    },
                  ],
                  createdAt: new Date().toISOString(),
      ...
      const content = result.current.messages[0]
        .content[0] as GenuiToolUseContent;
      // Input should only have non-_genui_ properties
      expect(content.input).toEqual({ location: "NYC", units: "celsius" });
      expect(content.input._genui_statusMessage).toBeUndefined();
      expect(content.input._genui_completionStatusMessage).toBeUndefined();
    });
                  ...
                  content: [
                    {
                      ...
                      name: "getCurrentTime",
                      input: undefined,
                    } as unknown as GenuiToolUseContent,
                  ],
                  createdAt: new Date().toISOString(),
                },
              ],
    ...
    it("handles multiple tool_use blocks with different completion states", () => {
      ...
      const tool2 = result.current.messages[0]
        .content[1] as GenuiToolUseContent;
  ...
  describe("cancelRun", () => {
    it("sets thread to idle and lastRunCancelled to true when cancelled", async () => {
      ...
      expect(mockGenuiClient.threads.runs.delete).toHaveBeenCalledWith(
        "run_456",
        { threadId: "thread_123" },
      );
    });
    ...
    it("is a no-op when there is no active run", async () => {
      ...
      expect(mockGenuiClient.threads.runs.delete).not.toHaveBeenCalled();
    });
    ...
    it("still updates local state even if API call fails", async () => {
      ...
      jest
        .mocked(mockGenuiClient.threads.runs.delete)
  ...
  describe("updateThreadName", () => {
    it("updates local thread name and invalidates caches on success", async () => {
      ...
      (mockGenuiClient.threads as any).update = mockUpdate;
