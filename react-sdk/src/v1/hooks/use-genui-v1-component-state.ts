...
/**
 * useGenuiComponentState - Component State Hook
 *
 * Provides bidirectional state synchronization between React components
 * and the Genui backend. State changes are debounced before syncing to
 ...
 *   provider wraps the component (e.g., first render before withGenuiInteractable)
 */
...
import { findComponentContent } from "@workspace/client";

/**
 * Return type for useGenuiComponentState hook.
 * Similar to useState but with additional metadata.
 */
export type UseGenuiComponentStateReturn<S> = [
  currentState: S,
 ...
 * to the Genui backend. Server-side state updates are also reflected
 * in the component.
 *
 ...
 * ```tsx
 * function Counter() {
 *   const [count, setCount, { isPending }] = useGenuiComponentState('count', 0);
 *
 *   return (
 *     <div>
 *       <span>{count}</span>
 ...
 * ```
 */
export function useGenuiComponentState<S = undefined>(
  keyName: string,
  initialValue?: S,
  debounceTime?: number,
): UseGenuiComponentStateReturn<S | undefined>;
export function useGenuiComponentState<S>(
  keyName: string,
  initialValue: S,
  debounceTime?: number,
): UseGenuiComponentStateReturn<S>;
export function useGenuiComponentState<S>(
  keyName: string,
  initialValue?: S,
  debounceTime = 500,
  ...
  // (before withGenuiInteractable sets the interactableId and wraps with provider).
  ...
  // (set by withGenuiInteractable). When context is missing entirely,
  ...
  const syncToServer = useDebouncedCallback(async (newState: S) => {
    ...
    try {
      ...
      setError(syncError);
      console.error(
        `[useGenuiComponentState] Failed to sync state for ${componentId}:`,
        syncError,
      );
    } finally {
