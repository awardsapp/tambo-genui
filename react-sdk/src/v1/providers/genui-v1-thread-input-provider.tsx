...
/**
 * GenuiThreadInputProvider - Shared Thread Input Context
 *
 * Provides shared input state across all components, enabling features like
 * suggestions to update the input field directly.
 *
 * This mirrors the GenuiThreadInputProvider pattern from the legacy beta SDK.
 */
...
import {
  useGenuiMutation,
  type UseGenuiMutationResult,
} from "../../hooks/react-query-hooks";
import { useGenuiSendMessage } from "../hooks/use-genui-v1-send-message";
import type { InputMessage } from "../types/message";
import type { ToolChoice } from "@workspace/client";
import { isPlaceholderThreadId } from "@workspace/client";
import { useGenuiAuthState } from "../hooks/use-genui-v1-auth-state";
import { useStreamDispatch, useStreamState } from "./genui-v1-stream-context";
 ...
 */
export interface GenuiThreadInputContextProps extends Omit<
  UseGenuiMutationResult<
 ...
 * @internal
 */
export const GenuiThreadInputContext = createContext<
  GenuiThreadInputContextProps | undefined
>(undefined);

/**
 * Provider that manages shared thread input state across all components.
 *
 * This ensures that useGenuiThreadInput, useGenuiSuggestions, and other components
 * all share the same input state.
 * @param props - Provider props
 * @param props.children - Child components
 * @returns Thread input context provider
 */
export function GenuiThreadInputProvider({ children }: PropsWithChildren) {
  ...
  const sendMessage = useGenuiSendMessage(currentThreadId);
  ...
  const contextValue: GenuiThreadInputContextProps = {
    ...mutationState,
    value: inputValue,
  ...
  return (
    <GenuiThreadInputContext.Provider value={contextValue}>
      {children}
 ...
 * @throws {Error} If used outside GenuiThreadInputProvider
 * @example
 * ```tsx
 * function ChatInput() {
 ...
 * ```
 */
export function useGenuiThreadInput(): GenuiThreadInputContextProps {
  const context = useContext(GenuiThreadInputContext);
  if (!context) {
    throw new Error(
      "useGenuiThreadInput must be used within GenuiThreadInputProvider",
    );
  }
  return context;
}
