...
import {
  ...
  useGenuiRegistry,
} from "./genui-registry-provider";

// Shared tool registry for all tests
const createMockTools = (): GenuiTool[] => [
  defineTool({
    name: "test-tool-1",
...
const createMockComponents = (tools: GenuiTool[]): GenuiComponent[] => [
  {
    name: "TestComponent",
...
describe("GenuiRegistryProvider", () => {
  ...
  describe("Component and tool registration", () => {
    it("should register components and tools correctly", () => {
      const wrapper = ({ children }: { children: React.ReactNode }) => (
        <GenuiRegistryProvider components={mockComponents}>
          {children}
        </GenuiRegistryProvider>
      );
    ...
    it("should handle tool registration and association", () => {
      const wrapper = ({ children }: { children: React.ReactNode }) => (
        <GenuiRegistryProvider>{children}</GenuiRegistryProvider>
      );
    ...
    it("should validate component schemas and throw error for invalid schemas", () => {
      const invalidComponent: GenuiComponent = {
  ...
  describe("Resource registration", () => {
    ...
    it("should throw error when only listResources is provided", () => {
      ...
      expect(() => {
        const wrapper = ({ children }: { children: React.ReactNode }) => (
          <GenuiRegistryProvider listResources={mockListResources}>
            {children}
          </GenuiRegistryProvider>
        );
        renderHook(() => useGenuiRegistry(), { wrapper });
      }).toThrow(
    ...
    it("should throw error when only getResource is provided", () => {
      ...
      expect(() => {
        const wrapper = ({ children }: { children: React.ReactNode }) => (
          <GenuiRegistryProvider getResource={mockGetResource}>
            {children}
          </GenuiRegistryProvider>
        );
        renderHook(() => useGenuiRegistry(), { wrapper });
