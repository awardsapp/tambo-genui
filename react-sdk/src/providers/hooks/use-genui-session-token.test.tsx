...
import { useGenuiSessionToken } from "./use-genui-session-token";
import type { PartialGenuiAI } from "../../testing/types";

describe("useGenuiSessionToken", () => {
  ...
  const mockAuthApi = {
    getToken: jest.fn(),
  } satisfies NonNullable<PartialGenuiAI["beta"]>["auth"];

  const mockBeta = {
    auth: mockAuthApi,
  } satisfies PartialGenuiAI["beta"];

  const mockGenuiAI = {
    apiKey: "",
    beta: mockBeta,
    bearer: "",
  } satisfies PartialGenuiAI as unknown as GenuiAI;
  ...
  it("should return null initially when no userToken is provided", () => {
    const { result } = renderHook(() =>
      useGenuiSessionToken(mockGenuiAI, queryClient, undefined),
    );
  ...
  it("should fetch and return session token when userToken is provided", async () => {
    ...
    const { result } = renderHook(() =>
      useGenuiSessionToken(mockGenuiAI, queryClient, "user-token"),
    );
    ...
    expect(mockGenuiAI.bearer).toBe(mockTokenResponse.access_token);
  ...
  it("should set bearer token on client", async () => {
    ...
    expect(mockGenuiAI.bearer).toBe("test-access-token");
  });
  ...
  it("should handle different token responses", async () => {
    ...
    expect(mockGenuiAI.bearer).toBe("custom-access-token");
  });
  ...
  it("should not fetch token when userToken changes to undefined", async () => {
    ...
    const { result, rerender } = renderHook(
      ({ userToken }) =>
        useGenuiSessionToken(mockGenuiAI, queryClient, userToken),
      {
