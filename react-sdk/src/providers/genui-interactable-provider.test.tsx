...
import { z } from "zod";
import { GenuiInteractableComponent } from "../model/genui-interactable";
import {
...
jest.mock("./genui-context-helpers-provider", () => ({
...
jest.mock("./genui-registry-provider", () => ({
  useGenuiRegistry: () => ({
...
describe("GenuiInteractableProvider - State Tracking", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  it("should set and get state for a component", () => {
    const { result } = renderHook(() => useGenuiInteractable(), { wrapper });

    const component: Omit<GenuiInteractableComponent, "id" | "createdAt"> = {
      name: "TestComponent",
...
describe("GenuiInteractableProvider - State Update Tool Registration", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  it("should register props update tool with genuiStreamableHint true by default", () => {
    const { result } = renderHook(() => useGenuiInteractable(), { wrapper });
    ...
    expect(propsToolCall[0].annotations).toEqual(
      expect.objectContaining({ genuiStreamableHint: true }),
    );
  });

  it("should register state update tool with genuiStreamableHint true by default", () => {
    const { result } = renderHook(() => useGenuiInteractable(), { wrapper });
  ...
  it("should allow overriding genuiStreamableHint via component annotations", () => {
    const { result } = renderHook(() => useGenuiInteractable(), { wrapper });
    ...
    expect(propsToolCall[0].annotations).toEqual(
      expect.objectContaining({ genuiStreamableHint: false }),
    );
    expect(stateToolCall[0].annotations).toEqual(
      expect.objectContaining({ genuiStreamableHint: false }),
    );
