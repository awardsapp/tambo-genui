...
import { useGenuiContextHelpers } from "./genui-context-helpers-provider";
...
export interface GenuiContextAttachmentProviderProps {
 ...
 * - **Included by default** in GenuiProvider - no need to wrap separately
 * - Use `useGenuiContextAttachment()` hook to manage context attachments
 * **What it does:**
 ...
 * @param props - The props for the GenuiContextAttachmentProvider
 * @param props.children - The children to wrap
 * @returns The GenuiContextAttachmentProvider component
 */
export function GenuiContextAttachmentProvider({
  children,
}: GenuiContextAttachmentProviderProps) {
 ...
 * **Must be used within a `GenuiProvider`** - throws an error otherwise.
 * @throws {Error} If used outside of GenuiProvider
 * @returns The context attachment state and methods
 * @example
 * ```tsx
 * const { addContextAttachment, attachments, clearContextAttachments } = useGenuiContextAttachment();
 *
 ...
 * ```
 */
export function useGenuiContextAttachment() {
  const context = useContext(ContextAttachmentContext);
  if (!context) {
    throw new Error(
      "useGenuiContextAttachment must be used within a GenuiContextAttachmentProvider",
    );
  }
  return context;
}
