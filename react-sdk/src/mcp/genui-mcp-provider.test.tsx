...
jest.mock("../providers/genui-registry-provider", () => {
  const actual = jest.requireActual("../providers/genui-registry-provider");
  return {
    ...actual,
    useGenuiRegistry: jest.fn(),
  };
});
...
jest.mock("../providers/genui-client-provider", () => {
  return {
    ...
    GenuiClientContext: React.createContext(undefined),
  };
});
...
const TestWrapper: React.FC<{
  ...
  return (
    ...
    >
      <GenuiRegistryProvider mcpServers={mcpServers}>
        <GenuiMcpTokenProvider>
          <GenuiMcpProvider handlers={handlers}>{children}</GenuiMcpProvider>
        </GenuiMcpTokenProvider>
      </GenuiRegistryProvider>
    </GenuiClientContext.Provider>
  );
...
describe("GenuiMcpProvider server list changes", () => {
  beforeEach(() => {
    // Mock registry so tool registration is a no-op
    (useGenuiRegistry as unknown as jest.Mock).mockReturnValue({
      registerTool: jest.fn(),
    });

    // Mock client with baseURL
    (useGenuiClient as unknown as jest.Mock).mockReturnValue({
      baseURL: "https://api.genui.co",
    });
  ...
  it("adds a new server when the list grows", async () => {
    ...
    const { rerender, getByTestId } = render(
      ...
      >
        <GenuiRegistryProvider mcpServers={["https://a.example"]}>
          <GenuiMcpTokenProvider>
            <GenuiMcpProvider>
  ...
  // Note: Token changes for the internal Genui server are covered by the
...
describe("GenuiMcpProvider handler support", () => {
  let mockClient: any;
...
describe("GenuiMcpProvider serverKey derivation and tool prefixing", () => {
