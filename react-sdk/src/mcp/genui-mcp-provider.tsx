...
import { useGenuiMcpToken } from "../providers/genui-mcp-token-provider";
import {
  useGenuiMcpServerInfos,
  useGenuiRegistry,
...
// Constant for the internal Genui MCP server name
const GENUI_INTERNAL_MCP_SERVER_NAME = "__genui_internal_mcp_server__";
 ...
 * and the internal Genui MCP server (when token is available).
...
function useServerConfigs(
  ...
  genuiBaseUrl: string | null | undefined,
  ...
  return useMemo(() => {
    ...
    // Add internal Genui MCP server if we have an access token and a base URL
    if (mcpAccessToken && genuiBaseUrl) {
      const base = new URL(genuiBaseUrl);
      base.pathname = `${base.pathname.replace(/\/+$/, "")}/mcp`;
      const genuiMcpUrl = base.toString();
      const tokenHash = hashString(mcpAccessToken);
      const internalServer: NormalizedMcpServerInfo = {
        name: GENUI_INTERNAL_MCP_SERVER_NAME,
        url: genuiMcpUrl,
        transport: MCPTransport.HTTP,
        serverKey: `genui-${tokenHash}`,
        customHeaders: {
      ...
      const serverInfo = normalizeServerInfo(
        internalServer,
        ServerType.GENUI_INTERNAL,
      );
      serverMap.set(serverInfo.key, serverInfo);
    }

    return serverMap;
  }, [mcpServers, mcpAccessToken, genuiBaseUrl]);
}
...
async function registerServerTools(
  ...
  registerTool: ReturnType<typeof useGenuiRegistry>["registerTool"],
): Promise<void> {
 ...
 * It automatically includes an internal Genui MCP server when an MCP access token is available.
 *
 * **BREAKING CHANGE**: This provider no longer accepts `mcpServers` as a prop.
 * Instead, pass `mcpServers` to `GenuiProvider` or `GenuiRegistryProvider`.
 * This provider must be wrapped inside `GenuiProvider` to access the MCP server registry.
 ...
 * @returns The GenuiMcpProvider component
 */
export const GenuiMcpProvider: FC<{
  handlers?: ProviderMCPHandlers;
  contextKey?: string;
  children: React.ReactNode;
}> = ({ handlers, contextKey, children }) => {
  const { registerTool } = useGenuiRegistry();
  const { mcpAccessToken, genuiBaseUrl } = useGenuiMcpToken(contextKey);
  const mcpServers = useGenuiMcpServerInfos();
 ...
 */
export const useGenuiMcpServers = () => {
  return useContext(McpProviderContext).servers;
};

/**
 * Hook to access MCP elicitation state from GenuiMcpProvider.
 * This provides access to the current elicitation request and methods to respond to it.
 *
 * The elicitation state is automatically managed by GenuiMcpProvider when MCP servers
 ...
 * ```
 */
export const useGenuiMcpElicitation = (): ElicitationContextState => {
